...
function XFunction(argnames, body, context) {
    ...
    this.argnames = function () { return argnames; };
}
...
function Interpreter(context) {
    ...
    this.processCall = function (node) {
        ...
        const argnames = fn.execute ? fn.argnames() : null;
        ...
        for (let k = 0, l = args.length; k < l; k++) {
            ...
            if (argnames && arg.ntype && arg.ntype() === 'named') {
                const p = argnames.indexOf(arg.name());
                values[p] = this.process(arg.expression());
            }
            else
                values[k] = this.process(arg);
        }
    ...
    this.processProperty = function (node) {
        const ctx = this.process(node.expression());
        
        return ctx.get(node.name());
    };
    
    this.processNamed = function (node) {
        const value = this.process(node.expression());
