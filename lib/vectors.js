...
function Vector(elems) {
    ...
    for (let k = 0, l = elems.length; k < l; k++) {
        ...
        if (elem && typeof elem.elements === 'function')
    ...
    // TODO return default value (according type) if index === 0
...
function VectorArray(elems, dims) {
    ...
    function hasManyCells(indexes) {
        for (let k = 0, l = indexes.length; k < l; k++)
            if (Array.isArray(indexes[k]))
               return true;
           
        return false;
    }
    
    function processCells(ndim, indexes, fn, coors) {
        if (ndim >= indexes.length)
            return fn(coors);
        
        const index = indexes[ndim];
        
        if (Array.isArray(index))
            for (let k = 0, l = index.length; k < l; k++) {
                coors[ndim] = index[k];
                processCells(ndim + 1, indexes, fn, coors);
            }
        else {
            coors[ndim] = index;
            processCells(ndim + 1, indexes, fn, coors);
    ...
    this.get = function () {        
        ...
        if (hasManyCells(indexes)) {
            const result = [];
            
            processCells(0, indexes, accum, []);
            
            function accum(coors) {
                result.push(self.get.apply(self, coors));
            }
...
function createArray(elems, dims) {
    ...
    else if (dims.length == 2 && dims[1] == null) {
        let ncols = Math.floor(elems.length / dims[0]);
        
        if (elems.length % dims[0])
            ncols++;
...
function createIntegerVector(length) {
...
function createDoubleVector(length) {
    length = Math.floor(length);
...
function createCharacterVector(length) {
    length = Math.floor(length);
...
function createLogicalVector(length) {
    length = Math.floor(length);
...
module.exports = {
    ...
    integer: createIntegerVector,
    double: createDoubleVector,
    character: createCharacterVector,
    logical: createLogicalVector
};
